...
public class MainApp {
	...
	public static void main(String[] args) 
	{
		...
		angajati
			.stream()
			.filter(angajat -> conditiiValide(angajat))
		...
		angajati
			.stream()
			.filter(angajat -> !(angajat.getPost().equals("sef")||angajat.getPost().equals("manager")))
			.sorted((Angajat a1, Angajat a2) -> -Float.compare(a1.getSalariu(), a2.getSalariu()))
			.collect(Collectors.toList())
			.stream()
			.forEach(System.out::println);
		...
		extrageSalarii(angajati, (Angajat a) -> a.getSalariu() < 3000)
			.stream()
			.forEach(System.out::println);
